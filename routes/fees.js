...
router.get('/fees', requireAuth, async (req, res) => {
  const invoices = await getInvoicesWithBalances(req.session.studentId);
  res.render('fees', { title: 'Fees', invoices, formatCents, error: null, success: null });
});

router.post('/fees/pay', requireAuth, verifyCsrfToken, async (req, res) => {
  const studentId = req.session.studentId;
  const invoiceId = parseInt(req.body.invoice_id, 10);
  const renderError = async (error) => {
    const invoices = await getInvoicesWithBalances(studentId);
  ...
  await db.run('INSERT INTO fee_payments (invoice_id, student_id, amount_cents) VALUES (?, ?, ?)', [
    invoiceId,
    studentId,
    amountCents,
  ]);

  const updatedInvoices = await getInvoicesWithBalances(studentId);
  res.render('fees', {
    title: 'Fees',
