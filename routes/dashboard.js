...
router.get('/dashboard', requireAuth, async (req, res) => {
  const student = await db.get('SELECT id, full_name, email FROM students WHERE id = ?', [
  ...
  const recentGrades = await db.all(
  ...
  const { total_due_cents: totalDueCents, total_paid_cents: totalPaidCents } = await db.get(
    `SELECT
